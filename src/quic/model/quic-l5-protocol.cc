  ...
  for (auto it = disgregated.begin (); it != disgregated.end (); ++it)
    {
      ...
      //std::cout<<"//****QuicL5Protocol::DispatchRecv offset: "<<sub.GetOffset()<<std::endl;
          ...
          or sub.IsStream ())
        {
          ...
          if (stream != nullptr
                   ...
                   == QuicStream::BIDIRECTIONAL))
            {
              ...
              m_currentOffset = stream->GetCurrentOffset();
              m_largestInOrderOffset = stream->GetLargestOffset();
              //std::cout<<">>> QuicL5Protocol::DispatchRecv--m_currentOffset: "<<m_currentOffset
                       // <<" m_expectedOffset: "<<m_largestInOrderOffset<<std::endl;
            }
        }
      else
        {
