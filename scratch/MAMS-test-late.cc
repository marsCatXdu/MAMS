...
int main (int argc, char *argv[])
{
    ...
    uint8_t schAlgo = 5;
    std::string maxBuffSize = "6p";
    uint64_t fileSize = 5e6;

    Config::SetDefault("ns3::QuicStreamBase::StreamSndBufSize",UintegerValue(50485760));
    Config::SetDefault("ns3::QuicStreamBase::StreamRcvBufSize",UintegerValue(50485760));
    Config::SetDefault("ns3::QuicSocketBase::SocketSndBufSize",UintegerValue(50485760));
    Config::SetDefault("ns3::QuicSocketBase::SocketRcvBufSize",UintegerValue(50485760));
    ...
    std::string fileNameWithNoExtension = "FlowVSThroughput_LATE_";
    ...
    if(isMob) {
        ...
        for(int i = 0; i < 100; i++) {
            ...
            bwMbps[1] = bwMbps[1] - 0.06;
            Simulator::Schedule(MilliSeconds(i*100), &ModifyLinkRate, &netDevices[0], echoClient, DataRate(std::to_string(bwMbps[0])+"Mbps"), 0);
            Simulator::Schedule(MilliSeconds(i*100), &ModifyLinkRate, &netDevices[1], echoClient, DataRate(std::to_string(bwMbps[1])+"Mbps"), 1);
        }
    }
